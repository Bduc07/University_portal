...
exports.getFeedbackForTeacher = async (req, res) => {
  ...
  try {
    ...
    // submitter_identifier is the student's users.id, so resolve a display name
    // via a join (falls back to the raw value for older string-based rows).
    const [feedbackResponses] = await pool.query(
      `SELECT fr.submitter_identifier, fr.question_id, fr.response_value, fr.created_at, fr.is_anonymous, fq.question_text,
       ...
       LEFT JOIN users u ON u.id = CAST(fr.submitter_identifier AS UNSIGNED)
       WHERE fr.teacher_id = ?`,
      [teacherId]
    );
    ...
    const groupedFeedback = feedbackResponses.reduce((acc, response) => {
      ...
      if (!acc[key]) {
        acc[key] = {
          studentId: response.student_name || response.submitter_identifier,
